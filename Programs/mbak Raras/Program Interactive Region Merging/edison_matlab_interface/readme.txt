...
use this checklist to ensure you 
create a complete deployment package.
You can distribute a MATLAB Compiler-generated 
standalone
application, component, or library to any target machine with 
the same operating 
system as the machine on which 
...
Runtime (MCR) is installed on target machines, 
and ensure you
...
Mac/Intel Mac Applications/MATLAB/MATLAB_Component_Runtime/Linux              

*** No predetermined installation location ***
                           
Contact your system administrator.  

         

*IMPORTANT: Always attempt to locate an existing MCR on your system before using the BUILDMCR 
command to create an MCR. If you must create an MCR, see the Compiler documentation for more 
details
 

B. MATLAB 2007a uses MCR version 7.6. 
To verify
...
A. Locate the name of the environment variable to set, using the table below:  
...
command prompt and issuing 
the DOS command:

 set PATH=C:\Program Files\MATLAB\MATLAB Component Runtime\v76;%PATH% or, add:

 
C:\Program Files\MATLAB\MATLAB Component Runtime\v76
 to the PATH environment variable by doing 
the following:
...
* Add the MCR directory to the environment variable by issuing this command:
 
setenv PATH /<matlabroot>/v76:"$PATH"


To make this change persistent after logout on UNIX machines, modify the .cshrc file to 
include this setenv command.
        
         
NOTE: On Windows, the Environment Variable syntax utilizes backslashes (\), delimited by 
semi-colons (;). On UNIX,
...
file run_edison_wrapper.sh on UNIX and Mac
...
archive - manual unzip 
required)

* edison_wrapper (application created by MATLAB Compiler)

* run_edison_wrapper.sh (shell script that can be run to temporarily set environment paths 
...
All Platforms: 
* edison_wrapper.so (shared library, file extension varies by platform)
...
* edison_wrapper.pdb (program database file - if DEBUG option selected)
...
* edison_wrapper_1_0.dll (component containing compiled M code)
